...
class FigmaTemplateConverter {
  ...
  async convertMultipleFiles(fileConfigs) {
    const results = [];
    ...
    for (const config of fileConfigs) {
      try {
        ...
        this.saveTemplate(template, config.outputPath);
        results.push({ success: true, template: template });
        
      } catch (error) {
        console.error(`Failed to convert ${config.name}:`, error);
...
// Example usage
async function main() {
  const converter = new FigmaTemplateConverter();
  
  // Example: Convert a Figma file
  // Replace 'YOUR_FILE_KEY' with actual Figma file key from URL
  const fileConfigs = [
    {
      fileKey: 'IMu8xF11cVEvg8TUs6NyxD', // Your Student Council Presentation template
      name: 'Student Council Presentation', // Template name
      category: 'education', // Perfect for students
...
module.exports = FigmaTemplateConverter;
