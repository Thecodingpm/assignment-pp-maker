...
export async function exportToPdf(document: StructuredDocument, options: ExportOptions = DEFAULT_EXPORT_OPTIONS): Promise<void> {
  ...
  try {
    ...
    // Save PDF
    const filename = `${options.filename || document.title || 'document'}.pdf`;
    pdf.save(filename);
