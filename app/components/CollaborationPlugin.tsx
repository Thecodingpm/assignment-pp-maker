...
export default function CollaborationPlugin({ 
  ...
  const checkActiveSession = async () => {
    try {
      ...
      setShareLink(`${window.location.origin}/assignment-editor/editor?session=${activeSession.sessionId}&collaborate=true&collab=lite`);
  ...
  const startCollaboration = async () => {
    ...
    try {
      ...
      setIsCollaborating(true);
      const link = `${window.location.origin}/assignment-editor/editor?session=${newSessionId}&collaborate=true&collab=lite`;
      setShareLink(link);
  ...
  const joinExistingSession = async () => {
    ...
    try {
      ...
      setIsCollaborating(true);
      const link = `${window.location.origin}/assignment-editor/editor?session=${joinSessionId.trim()}&collaborate=true&collab=lite`;
      setShareLink(link);
      try { await navigator.clipboard.writeText(link); setSuccess('Link copied to clipboard'); } catch {}
      router.replace(link);
