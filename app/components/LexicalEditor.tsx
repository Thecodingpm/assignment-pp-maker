                ...
                if (tagName === 'h1' || tagName === 'h2' || tagName === 'h3' || tagName === 'h4' || tagName === 'h5' || tagName === 'h6') {
                  ...
                  Array.from(child.children).forEach((listItem) => {
                    if (listItem.tagName.toLowerCase() === 'li') {
                      const paragraphNode = $createParagraphNode();
                      paragraphNode.append($createTextNode(listItem.textContent || ''));
                      root.append(paragraphNode);
                    }
                  });
                  ...
                  // Apply div styles
                  setTimeout(() => {
                    const divElement = document.querySelector(`[data-lexical-key="${paragraphNode.getKey()}"]`) as HTMLElement;
                    if (divElement) {
        ...
        editor.update(() => {
          ...
          root.getChildren().forEach((node) => {
            if (node.getType() === 'heading') {
              ...
              const headingElement = document.querySelector(`[data-lexical-key="${node.getKey()}"]`) as HTMLElement;
              if (headingElement) {
                headingElement.style.color = '#2563eb';
                headingElement.style.textAlign = 'center';
                headingElement.style.marginBottom = '20px';
              ...
              // Apply paragraph styles
              const paragraphElement = document.querySelector(`[data-lexical-key="${node.getKey()}"]`) as HTMLElement;
              if (paragraphElement) {
                paragraphElement.style.fontSize = '16px';
                paragraphElement.style.lineHeight = '1.6';
                paragraphElement.style.marginBottom = '15px';
