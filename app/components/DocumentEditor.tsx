...
const DocumentEditor: React.FC<DocumentEditorProps> = ({
  ...
  const [document, setDocument] = useState<StructuredDocument>(initialDocument);
  ...
  const handleBlockChange = useCallback((blockId: string, content: string, styling: DocumentBlock['styling']) => {
    const result = updateBlock(document, blockId, content, styling);
  ...
  const handleBlockDelete = useCallback((blockId: string) => {
    const result = deleteBlock(document, blockId);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  ...
  const handleBlockMove = useCallback((fromIndex: number, toIndex: number) => {
    const result = moveBlock(document, fromIndex, toIndex);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  }, [document]);
  ...
  const handleBlockTypeChange = useCallback((blockId: string, newType: DocumentBlock['type']) => {
    const result = changeBlockType(document, blockId, newType);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  }, [document]);
  ...
  const handleImageReplace = useCallback((blockId: string, newImageUrl: string) => {
    const result = updateBlock(document, blockId, undefined, { ...document.blocks.find(b => b.id === blockId)?.styling });
  ...
  const handleAddBlock = useCallback((type: DocumentBlock['type'] = 'paragraph', index?: number) => {
    ...
    const insertIndex = index !== undefined ? index : document.blocks.length;
    const result = addBlock(document, newBlock, insertIndex);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  ...
  }, [document, onSave, hasUnsavedChanges]);
  ...
  const handleBlockSplit = useCallback((blockId: string, splitIndex: number) => {
    const result = splitBlock(document, blockId, splitIndex);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  }, [document]);
  ...
  const handleBlockMerge = useCallback((blockId: string) => {
    const result = mergeBlock(document, blockId);
    setDocument(result.document);
    setHistory(prev => addToHistory(prev, result.operation));
    setHasUnsavedChanges(true);
  }, [document]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const handled = handleKeyboardShortcut(event, document, selectedBlockId, {
  ...
  }, [isEditing, document, selectedBlockId, handleBlockDelete, handleBlockDuplicate, handleBlockSplit, handleBlockMerge, handleUndo, handleRedo]);
