...
const MediaPopup: React.FC<MediaPopupProps> = ({
  ...
  const fetchMedia = async (query: string, page: number) => {
    if (!query.trim() && selectedIntegration !== 'tenor') {
      return; // Don't fetch empty queries for non-Tenor integrations
    }
    ...
    try {
      ...
      } else if (selectedIntegration === 'tenor') {
        endpoint = '/api/tenor';
      ...
      console.log(`Fetching from ${selectedIntegration}: ${endpoint}${params}`);
      ...
      console.log(`${selectedIntegration} response:`, data);
  ...
  const handleIntegrationSwitch = (integrationId: string) => {
    ...
    if (integrationId === 'tenor') {
      fetchMedia('trending', 1); // Load trending GIFs for Tenor
    } else if (integrationId === 'unsplash') {
      fetchMedia('nature', 1); // Load nature photos for Unsplash
      ...
      fetchMedia('', 1); // Default for other integrations
    }
  };
