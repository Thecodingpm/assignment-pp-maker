...
function Navbar() {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Get content from all lexical editors (for multi-page support)
      ...
      if (editorElements && editorElements.length > 0) {
        // Combine content from all editors
        const contents = Array.from(editorElements).map((editor, index) => {
        ...
        allContent = editorElement ? editorElement.innerHTML : '';
      }
